...
function getRecognitionLanguage(lang: VoiceLang) {
  // ar-EG covers Egyptian + most informal/chat Arabic far better than ar-SA,
  // and Chrome/Edge fall back to MSA seamlessly when needed.
...
export function VoiceInput({
  ...
  const startWaveform = (stream: MediaStream) => {
    ...
    // some mobile browsers start the context suspended until a user gesture resumes it
    ...
    // rolling history of recent amplitudes — silence reads as a flat line, speech as peaks
    const history: number[] = Array.from({ length: 28 }, () => 0);
    // noise gate to ignore ambient mic hiss (especially loud on phones)
    ...
    const draw = () => {
      ...
      const gated = rms < NOISE_FLOOR ? 0 : Math.min(1, (rms - NOISE_FLOOR) * 4.5);
      history.push(gated);
      history.shift();
      setLevels(history.map((amp) => Math.max(0.06, amp)));
  ...
  const startSyntheticWaveform = () => {
    ...
    const tick = () => {
      ...
      const next = Array.from({ length: 28 }, (_, i) => {
        const base = 0.25 + 0.5 * Math.abs(Math.sin(t * 4 + i * 0.55));
        const jitter = Math.random() * 0.2;
        return Math.min(1, Math.max(0.12, base + jitter - 0.1));
      });
      setLevels(next);
  ...
  const startRecording = (langOverride?: VoiceLang) => {
    setPanelOpen(true);
    setError(null);
    ...
    try {
      ...
      // Mobile browsers do not honor continuous=true reliably; restart on onend instead.
